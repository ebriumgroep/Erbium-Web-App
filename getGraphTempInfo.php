        ...
        if(dateExists($date, $finalArray)){
            for ($j = 0; $j < count($finalArray); $j++) {
                if ($finalArray[$j]['label'] == $date) {
                    $finalArray[$j]['value']+= $row['temperature'];
                    $finalArray[$j]['numTemps']++;
                    break;
                }
            }
        }else{
            ...
            $tempArray['value'] = $row['temperature'];
            $tempArray['numTemps'] = 1;
...
for ($i = 0; $i < count($finalArray); $i++){
    $finalArray[$i]['value'] = $finalArray[$i]['value']/$finalArray[$i]['numTemps'];
}
